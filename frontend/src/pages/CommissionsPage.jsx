...
const CommissionsPage = () => {
    ...
    useEffect(() => {
        // Carrega a lista de colaboradores para o dropdown
        api.get('/staff')
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-6">Relatório de Comissões</h1>

            {/* Filtros */}
            <div className="p-4 bg-white rounded-lg shadow-md mb-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700">Colaborador</label>
                    <select value={selectedStaff} onChange={(e) => setSelectedStaff(e.target.value)} className="mt-1 block w-full p-2 border rounded-md">
                ...
                <div className="md:col-span-4 text-right">
                     <button onClick={handleGenerateReport} disabled={loading} className="px-6 py-2 bg-purple-700 text-white font-semibold rounded-lg shadow-md hover:bg-purple-800 disabled:bg-gray-400">
            ...
            {report && (
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-xl font-bold mb-4">Resultado para: {report.staffName}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 text-center">
                        <div className="p-4 bg-gray-50 rounded"><p className="text-sm text-gray-500">Total Vendido</p><p className="text-2xl font-bold">{formatCurrency(report.totalSales)}</p></div>
                        <div className="p-4 bg-gray-50 rounded"><p className="text-sm text-gray-500">Taxa de Comissão</p><p className="text-2xl font-bold">{report.commissionRate}%</p></div>
                        <div className="p-4 bg-green-50 rounded"><p className="text-sm text-green-700">Comissão a Pagar</p><p className="text-2xl font-bold text-green-800">{formatCurrency(report.totalCommission)}</p></div>
                    </div>
                    <h3 className="font-semibold mb-2">Vendas Incluídas no Cálculo</h3>
